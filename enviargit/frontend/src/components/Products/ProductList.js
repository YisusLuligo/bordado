...
const ProductList = () => {
  ...
  const handleDeleteProduct = async (product) => {
    ...
    const stockInfo = product.cantidad_actual > 0 
      ? `\n⚠️ ATENCIÓN: Este producto tiene ${Math.floor(product.cantidad_actual)} unidades en stock.`
      : '';
    
    const valorInventario = product.cantidad_actual * product.precio_compra;
    const valorInfo = valorInventario > 0 
      ? `\n💰 Valor del inventario: ${formatCurrency(valorInventario)}`
      : '';

    const confirmMessage = `¿Estás seguro de eliminar el producto "${product.nombre}"?` +
      stockInfo +
      valorInfo +
                ...
                {categories.length === 0 ? (
                  <div>
                    <p style={{ color: '#f59e0b', marginBottom: '1rem' }}>
                      💡 Primero necesitas crear categorías para organizar tus productos
                    </p>
                    <button 
                      className="btn btn-secondary" 
                      onClick={() => setShowCategoryModal(true)}
