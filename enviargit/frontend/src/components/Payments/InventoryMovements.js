...
const InventoryMovements = () => {
  ...
  const calculateSummary = () => {
    return {
      ...
      entradas: filteredMovements.filter(m => ['entrada', 'devolucion'].includes(m.tipo_movimiento)).length,
      salidas: filteredMovements.filter(m => ['salida_venta', 'salida_pedido'].includes(m.tipo_movimiento)).length,
      ajustes: filteredMovements.filter(m => m.tipo_movimiento === 'ajuste').length
    };
  };
        ...
        <div className="card">
          ...
          <div style={{ space: '0.5rem' }}>
            {filteredMovements.map((movement) => {
              ...
              const isPositive = ['entrada', 'devolucion'].includes(movement.tipo_movimiento);
