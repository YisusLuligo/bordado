...
const ClientList = () => {
  ...
  const loadClients = async () => {
    try {
      ...
      // ⭐ NUEVO: Notificación de error al cargar
      showError(
        'Error de conexión',
        'No se pudieron cargar los clientes. Verifica tu conexión e inténtalo de nuevo.',
        { duration: 5000 }
      );
    } finally {
      setLoading(false);
  ...
  // ⭐ ACTUALIZADO: Función de eliminar con notificaciones
  const handleDeleteClient = async (client) => {
    // Información detallada antes de confirmar
    const hasOrders = client.ultima_compra; // Si tiene última compra, probablemente tenga pedidos
    const warningText = hasOrders 
      ? `\n⚠️ ATENCIÓN: Este cliente tiene historial de compras.`
      : '';
    
    const confirmMessage = `¿Estás seguro de eliminar el cliente "${client.nombre}"?` +
      warningText +
      `\n\n❌ Esta acción no se puede deshacer.` +
      `\n📱 Teléfono: ${client.telefono}` +
      (client.email ? `\n📧 Email: ${client.email}` : '') +
      `\n💼 Tipo: ${client.tipo_cliente}` +
      (client.descuento_especial > 0 ? `\n💰 Descuento: ${client.descuento_especial}%` : '') +
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        // ⭐ NUEVO: Actualizar lista inmediatamente (optimistic update)
        setClients(prev => prev.filter(c => c.id !== client.id));
        
        // ⭐ NUEVO: Mostrar notificación de éxito
        showSuccess(
          '¡Cliente eliminado exitosamente!',
          `${client.nombre} ha sido eliminado del sistema. Todos sus datos han sido borrados permanentemente.`,
          { duration: 5000 }
        );
        ...
        // ⭐ NUEVO: Revertir cambio optimista si falló
        loadClients();
        
        // ⭐ NUEVO: Manejo de errores específicos
        let errorMessage = 'Error eliminando cliente';
        if (err.response?.status === 400) {
          if (err.response.data?.pedidos_asociados) {
            errorMessage = `No se puede eliminar "${client.nombre}" porque tiene ${err.response.data.pedidos_asociados} pedido(s) asociado(s). Elimina primero los pedidos o márcalos como entregados.`;
          } else {
            errorMessage = 'No se puede eliminar el cliente porque tiene pedidos asociados';
          }
  ...
  // ⭐ ACTUALIZADO: Función de éxito con notificaciones
  const handleFormSuccess = () => {
    ...
    // La notificación la maneja el ClientForm
  };
