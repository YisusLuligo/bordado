...
const OrderDetail = ({ order, onClose, onEdit, onAddPayment, onReload, onChangeState }) => {
  ...
  // Manejo de cambio de estado con notificaciones
  const handleStateChange = async () => {
    if (!newState) return;
    
    setChangingState(true);
    try {
      const response = await pedidosAPI.cambiarEstado(order.id, { estado: newState });
      ...
      if (response.data.success && response.data.notificacion) {
        ...
        showSuccess(
          'Estado Actualizado',
          response.data.mensaje || `Estado cambiado a ${newState}`,
          { duration: 5000 }
        );
      }
      
      // Callback externo para cambio de estado
      if (onChangeState) {
        onChangeState(order, newState);
      }
      ...
      const errorMessage = error.response?.data?.error || 'Error cambiando estado';
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '1.5rem' }}>
            ...
            {nextStates.length > 0 && (
              <div style={{
                ...
                {/* Botones de cambio rápido para estados comunes */}
                <div style={{ marginBottom: '1rem' }}>
                    ...
                    {nextStates.map(state => {
                      ...
                      return (
                        <button
                          key={state}
                          onClick={() => {
                            if (window.confirm(`¿Cambiar estado a "${stateInfo.label}"?`)) {
                              setNewState(state);
                              // Auto-ejecutar el cambio
                              setTimeout(() => {
                                setNewState(state);
                                handleStateChange();
                              }, 100);
                ...
                {/* Selector tradicional para casos especiales */}
                ...
                <div style={{
                  ...
                  <strong>💡 Flujo de estados:</strong> Recibido → En Proceso → Terminado → Entregado
                </div>
              </div>
            )}

            {/* Mensaje si no hay cambios de estado disponibles */}
            {nextStates.length === 0 && (
              <div style={{
                backgroundColor: currentStateInfo.bgColor,
                ...
                <CurrentStateIcon size={48} color={currentStateInfo.color} style={{ margin: '0 auto 1rem' }} />
                <h3 style={{ color: currentStateInfo.color, marginBottom: '0.5rem', fontSize: '1.125rem' }}>
                  {currentStateInfo.label}
                </h3>
                <p style={{ color: currentStateInfo.color, margin: 0 }}>
                  {currentStateInfo.description}
                  {order.estado === 'entregado' && '. ¡Pedido completado exitosamente!'}
                  {order.estado === 'cancelado' && '. Para reactivar, usa el botón "Editar".'}
                </p>
              </div>
            )}
          </div>
