...
const OrderList = () => {
  ...
  // Función para cambiar estado con notificaciones
  const handleChangeState = async (order, newState) => {
    try {
      ...
      if (response.data.success && response.data.notificacion) {
        ...
        // Fallback si no hay notificación estructurada
        showSuccess(
          'Estado Actualizado',
      ...
      const errorMessage = error.response?.data?.error || 'Error cambiando estado del pedido';
  ...
  const getNextQuickState = (currentState) => {
    const stateFlow = {
      'recibido': 'en_proceso',
    ...
    <>
      ...
      <div style={{ maxWidth: '1400px' }}>
        ...
        {filteredOrders.length === 0 ? (
          ...
          <div className="card">
            {/* Resumen - Sin contar estados eliminados */}
            <div style={{ 
              display: 'flex', 
              justifyContent: 'space-between', 
              alignItems: 'center',
              ...
              <div style={{ display: 'flex', gap: '1rem', fontSize: '0.875rem' }}>
                <span style={{ color: '#2563eb' }}>
                  ● {filteredOrders.filter(o => o.estado === 'recibido').length} Recibidos
                </span>
                <span style={{ color: '#dc2626' }}>
                  ● {filteredOrders.filter(o => o.estado === 'en_proceso').length} En Proceso
                </span>
                <span style={{ color: '#7c3aed' }}>
                  ● {filteredOrders.filter(o => o.estado === 'terminado').length} Terminados
                </span>
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      {/* Acciones - Botón de cambio rápido de estado */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginLeft: '1rem' }}>
                        ...
                        {getNextQuickState(order.estado) && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              const nextState = getNextQuickState(order.estado);
                              if (nextState && window.confirm(`¿Cambiar a "${getStateInfo(nextState).label}"?`)) {
                                handleChangeState(order, nextState);
                              }
                            }}
                            style={{
                              background: getStateInfo(getNextQuickState(order.estado)).color,
                              border: 'none',
                              cursor: 'pointer',
                              padding: '0.5rem',
                              color: 'white',
                            ...
                            title={`Cambiar a ${getStateInfo(getNextQuickState(order.estado)).label}`}
                          >
                            <Settings size={14} />
                            <span style={{ fontSize: '0.75rem' }}>
                              {getStateInfo(getNextQuickState(order.estado)).label}
                            </span>
                          </button>
                        )}
