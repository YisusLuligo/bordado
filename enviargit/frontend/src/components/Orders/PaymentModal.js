// components/Orders/PaymentModal.js - VERSIÓN ARREGLADA
...
const PaymentModal = ({ order, onSuccess, onCancel }) => {
  ...
  const handleChange = (e) => {
    ...
    // ⭐ NUEVO: Para el campo monto, solo permitir números enteros
    if (name === 'monto') {
      // Solo permitir números enteros (sin decimales)
      const intValue = value.replace(/[^\d]/g, ''); // Solo dígitos
      setFormData(prev => ({
        ...prev,
        [name]: intValue
      }));
  ...
  const validateForm = () => {
    ...
    // ⭐ NUEVO: Validación para números enteros solamente
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ⭐ NUEVO: Convertir a entero antes de enviar
      const montoInt = parseInt(formData.monto);
      
      const dataToSend = {
        pedido: order.id,
        monto: montoInt, // Solo números enteros
  ...
  const calculateNewBalance = () => {
    const paymentAmount = parseInt(formData.monto) || 0; // ⭐ NUEVO: parseInt en lugar de parseFloat
          ...
          <form onSubmit={handleSubmit} style={{ flex: 1, overflow: 'auto' }}>
            <div style={{ padding: '1.5rem' }}>
              ...
              <div className="form-group">
                <label className="form-label">Monto del Pago * (solo números enteros)</label>
                <input
                  type="text" // ⭐ NUEVO: Cambiado de "number" a "text" para mejor control
                  name="monto"
                ...
                {/* ⭐ NUEVO: Información sobre solo enteros */}
                <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.25rem' }}>
                  Solo se permiten números enteros (sin decimales). Ej: 10000, 50000, 100000
                </div>
