...
from .models import VentaDirecta, DetalleVentaDirecta, PagoPedido, MovimientoInventario, Devolucion
from .serializers import (
    VentaDirectaSerializer, PagoPedidoSerializer, MovimientoInventarioSerializer,
    ResumenFinancieroSerializer, ProductoVentasSerializer, DevolucionSerializer
)
...
class VentaDirectaViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """Crear venta directa con manejo de transacciones"""
        try:
                        ...
                        return Response(
                            {
                                'error': f'Stock insuficiente para {producto.nombre}. '
                                        f'Disponible: {producto.cantidad_actual}, Solicitado: {cantidad}'
                            },
                            status=status.HTTP_400_BAD_REQUEST
                        )
            ...
            detalles_creados = []
            for detalle_data in detalles_data:
                detalle = DetalleVentaDirecta.objects.create(
                    venta=venta,
            ...
            # Actualizar última compra del cliente si existe
            if venta.cliente:
                venta.cliente.ultima_compra = timezone.now().date()
                venta.cliente.save(update_fields=['ultima_compra'])
            
            # Serializar la respuesta con los detalles
            response_serializer = self.get_serializer(venta)
            return Response(response_serializer.data, status=status.HTTP_201_CREATED)
            
        except Exception as e:
            return Response(
                {'error': f'Error creando la venta: {str(e)}'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
...
class PagoPedidoViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """Actualizar saldo automáticamente al crear pago"""
...
# ⭐ NUEVO: ViewSet para devoluciones
class DevolucionViewSet(viewsets.ModelViewSet):
    queryset = Devolucion.objects.select_related('producto')
    ...
    @transaction.atomic
    def create(self, request, *args, **kwargs):
        """Crear devolución con manejo de stock y notificaciones"""
        try:
            # Validar datos
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            ...
            # Crear la devolución (esto actualizará el stock automáticamente)
            ...
            return Response({
                ...
                'mensaje': f'Devolución procesada: {cantidad} unidades de {producto.nombre}',
                'stock_actualizado': {
                    'producto': producto.nombre,
                    'nuevo_stock': float(producto.cantidad_actual),
                    'cantidad_devuelta': float(cantidad)
                },
                # ⭐ Información para notificación
                'notificacion': {
                    'tipo': 'success',
                    'titulo': 'Devolución Procesada Exitosamente',
                    'mensaje': f'Se devolvieron {cantidad} unidades de {producto.nombre}. Stock actualizado automáticamente.',
                    'icono': '🔄',
                    'color': '#059669',
                    'mostrar_por': 6000
                }
...
class MovimientoInventarioViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # ⭐ ACTUALIZADO: Solo mostrar tipos permitidos
        queryset = queryset.filter(tipo_movimiento__in=['salida_venta', 'devolucion'])
        ...
        if tipo_movimiento and tipo_movimiento in ['salida_venta', 'devolucion']:
...
class DashboardFinancieroViewSet(viewsets.ViewSet):
    ...
    # ⭐ NUEVA: Endpoint para crear devolución (alias para facilitar frontend)
    @action(detail=False, methods=['post'])
    def crear_devolucion(self, request):
        """Endpoint simplificado para crear devoluciones desde frontend"""
        devolucion_viewset = DevolucionViewSet()
        devolucion_viewset.request = request
        return devolucion_viewset.create(request)
