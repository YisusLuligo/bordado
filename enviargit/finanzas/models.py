...
# ⭐ NUEVO: Modelo para devoluciones

class Devolucion(models.Model):
    """
    Registro de devoluciones de productos por parte de clientes
    """
    producto = models.ForeignKey(Producto, on_delete=models.CASCADE, related_name='devoluciones')
    cantidad = models.DecimalField(max_digits=8, decimal_places=2)
    motivo = models.CharField(max_length=500)
    cliente = models.CharField(max_length=200)  # Nombre del cliente (no FK para flexibilidad)
    venta_original = models.CharField(max_length=100, blank=True)  # Referencia opcional
    fecha_devolucion = models.DateTimeField(default=timezone.now)
    usuario = models.CharField(max_length=100, blank=True)  # Quien procesó la devolución
    
    def save(self, *args, **kwargs):
        # Si es una nueva devolución, actualizar stock automáticamente
        if not self.pk:
            ...
            MovimientoInventario.objects.create(
                ...
                cantidad=self.cantidad,  # Cantidad positiva (entrada)
                cantidad_anterior=cantidad_anterior,
                cantidad_nueva=self.producto.cantidad_actual,
                devolucion=self,
                motivo=f'Devolución: {self.motivo}',
                usuario=self.usuario or 'Sistema'
            )
        else:
            # Si ya existe, solo actualizar campos básicos
            super().save(*args, **kwargs)
    
    def __str__(self):
        return f"Devolución: {self.producto.nombre} x{self.cantidad} - {self.cliente}"
...
class MovimientoInventario(models.Model):
    """
    Historial de movimientos de inventario - SIMPLIFICADO
    """
    ...
    devolucion = models.ForeignKey(Devolucion, on_delete=models.SET_NULL, null=True, blank=True)  # ⭐ NUEVO
